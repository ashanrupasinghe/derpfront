...
<!-- BEGIN Main Container -->  
    ...
    if ($return['status']==0):
    if (sizeof($return['result']['product_list'])>0):?>
...
<tr class="<?php if ($count==0){echo 'first last ';}?>odd <?php if ($count==sizeof($return['result']['product_list'])-1){echo ' last';}?>">
    ...
    <td>
        ...
        <input name="cart[26340][qty]" value="<?php echo $product['quantity'];?>" size="4" title="Qty" class="input-text qty" maxlength="12" disabled>
    </td>
...
<p class="nothing-found"><?php echo $return ['message'];?></p>
<?php endif;?>        
    </form>
</div>
